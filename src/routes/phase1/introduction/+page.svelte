<script>
  ...
  onMount(async () => {
    progressData = await progressStore.getProgress();
  });
  ...
  const sections = [
    ...
    },
    {
      ...
      description: 'WebSocketの11の主要利用例を視覚的に理解し、実際のアプリケーション設計に活用する',
      duration: '90分',
      difficulty: '初級',
  ...
  function getLessonStatus(lessonId) {
    return progressData?.lessons?.[lessonId]?.completed ? 'completed' : 'pending';
  }
...
<svelte:head>
  ...
  <meta name="description" content="WebSocketの基本概念から実践的な利用例まで、体系的に学習するWebSocket入門セクション" />
</svelte:head>
...
graph TD
            ...
            <div class="flex items-start justify-between">
              <div class="flex-1">
                <div class="flex items-center gap-4 mb-4">
                  <div class="flex items-center justify-center w-12 h-12 bg-blue-100 text-blue-600 rounded-xl font-bold text-lg">
                    {section.number}
                  </div>
                  <div>
                ...
                <div class="flex flex-wrap gap-4 mb-6">
                  ...
                  {#if progressData}
                    <div class="flex items-center gap-2 text-sm {getStatusColor(getLessonStatus(section.lessonId))}">
    ...
    <!-- 次のフェーズへの案内 -->
    <div class="mt-16 bg-gradient-to-r from-purple-50 to-blue-50 rounded-xl p-8 border border-purple-200">
      <div class="text-center">
        <h2 class="text-2xl font-bold text-gray-900 mb-4">
          🎯 Phase 1完了後は...
        </h2>
        <p class="text-gray-600 mb-6 max-w-2xl mx-auto">
          WebSocketの基礎理解を完了すると、Phase 2でネットワーク技術とプロトコルの詳細、Phase 3で実践的な実装手法を学習していきます。
        </p>
